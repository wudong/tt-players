...
export function HomeView() {
    ...
    const { data: searchResults, isLoading } = usePlayerSearch(query);
        ...
        <div className="flex min-h-screen flex-col bg-slate-50 font-sans text-slate-900 pb-28">
            <header className="relative w-full rounded-b-[2.5rem] bg-gradient-to-br from-emerald-600 via-teal-700 to-slate-800 px-6 pb-8 pt-16 shadow-lg">
                ...
                <div className="relative mb-6 flex items-center justify-between">
                    ...
                    <button className="flex h-11 w-11 items-center justify-center rounded-full bg-white/20 text-white backdrop-blur-md transition-all hover:bg-white/30 hover:scale-105 active:scale-95 shadow-sm">
                        <Filter size={20} />
                    </button>
                </div>
            ...
            <main className="flex-1 px-5 pt-8 z-10 -mt-2">
                <div className="mb-4 flex items-end justify-between px-1">
                    <h2 className="flex items-center gap-2 text-lg font-bold text-slate-800">
                        {query ? <><Search size={20} className="text-emerald-500" /> Search Results</> : <><TrendingUp size={20} className="text-emerald-500" /> Trending Players</>}
                    </h2>
                </div>
