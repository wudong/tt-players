  ...
  return (
    <MenuTrigger>
      ...
      <Menu onAction={(action) => alert(`Triggering ${action}`)}>
        ...
        <UnavailableMenuItemTrigger {...props}>
...
| Name | Type | Default | Description |
|------|------|---------|-------------|
| `children` | `ReactElement<unknown, string | JSXElementConstructor<any>>[]` | — | The contents of the UnavailableMenuItemTrigger. The first child should be a MenuItem and the second child be a ContextualHelpPopover. |
